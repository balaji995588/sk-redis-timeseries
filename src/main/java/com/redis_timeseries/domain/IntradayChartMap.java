...
//
//import com.redis_timeseries.utility.FeedInfo;
//
//
//
...
//
//	//List<GraphSummerizedTicks> tickData = Collections.synchronizedList(new ArrayList<GraphSummerizedTicks>());
//	//List<ChartData> minuteData = Collections.synchronizedList(new ArrayList<ChartData>());
//	
//	//List<GraphSummerizedTicks> tickData = null;
//	public ConcurrentMap<Long, FeedInfo> tickDataMap = null;
//	public ConcurrentMap<Long, FeedInfo> getTickDataMap() {
//		return tickDataMap;
//	}
//
//	public void setTickDataMap(ConcurrentMap<Long, FeedInfo> tickDataMap) {
...
//		try {
//			if (startindex != 0) {
//				//newData = tickDataMap.values().stream().filter(cData -> cData.getLtt() > startindex).collect(Collectors.toList());
//				newData = tickDataMap.values().stream()
//											  .filter(cData -> cData.getLtt() > startindex)
//											  .sorted(Comparator.comparingLong(GraphSummerizedTicks::getLtt))
//											  .collect(Collectors.toList());
//			} else {
...
//					newData = tickDataMap.values().stream().filter(cData -> cData.getLtt() > startIndex).collect(Collectors.toList());
//				}else{
//					newData = tickDataMap.values().stream().filter(cData -> ((cData.getLtt() > startIndex)&&(cData.getLtt()<endIndex))).collect(Collectors.toList());
//				}
//		} catch (Exception e) {
//			LOGGER.error("Exception in intraday Tick data fetching process | " + e);
//		}
